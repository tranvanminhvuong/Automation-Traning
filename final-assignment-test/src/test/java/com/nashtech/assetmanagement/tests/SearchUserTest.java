...
import static com.nashtech.assetmanagement.repos.ManageUserFunction.searchUserListWithKeyWord;
...
public class SearchUserTest extends BaseTest {
    ...
    public void searchUser(ITestContext context, JsonObject jsonObject) throws InterruptedException, SQLException {
        ...
        JsonArray getUserListFollowLocationInDB = searchUserListWithKeyWord(criteria);
        Response response = accountHelper.searchUser(userToken, 1, limit, location, criteria);
        int pageCount = response.jsonPath().getInt("pageCount");
        manageUserPage.searchUserWithCriteria(criteria);
        // data in ui
        JsonArray getUserListInUI = new JsonArray();

        for (int i = 1; i <= pageCount; i++) {
            response = accountHelper.searchUser(userToken, i, limit, location, criteria);
    ...
    public Object[][] getData() throws FileNotFoundException {
        JsonObject jsonObject = readJsonFile("src/test/resources/testdata/search_user/data_search.json");
